...
const BUILTIN_VARIABLE_REGEX = /\{\{\$(\w+)\}\}/g;

function resolveBuiltinVariables(template: string): string {
  return template.replace(BUILTIN_VARIABLE_REGEX, (match, name: string) => {
    switch (name) {
      case 'timestamp': return String(Math.floor(Date.now() / 1000));
      case 'isoTimestamp': return new Date().toISOString();
      case 'uuid': return crypto.randomUUID();
      case 'randomInt': return String(Math.floor(Math.random() * 1000000));
...
export function resolveAll(
  ...
  const afterEnv = resolveVariables(afterNodes, variables);
  return resolveBuiltinVariables(afterEnv);
}
