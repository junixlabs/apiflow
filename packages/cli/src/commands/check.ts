...
function main(): void {
  ...
  // cm:guard Exit 2, never 1: 1 is "the map drifted", so an unreadable or malformed file reported as
  // 1 makes CI blame the code for a broken map — and it used to, as a raw stack trace.
  let stored: ApiMapFile;
  try {
    stored = parseMap(readFileSync(mapPath, 'utf8'));
  } catch (e) {
    console.error(`Cannot read ${mapPath}: ${e instanceof Error ? e.message : String(e)}`);
    process.exit(2);
    return;
  }
