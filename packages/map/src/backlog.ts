...
//
// Normalization is three narrow rules, one per interpolating producer — not a general normalizer. A
// regex that erased "anything variable-looking" would fold two different causes into one shape, and a
// ranking that points at a shape nobody wrote is worse than no ranking.
...
export function unresolvedShape(reason: string): string {
  let shape = reason;
  const prefix = /^(\S+) \S+ — (.+)$/.exec(shape);
  if (prefix !== null && VERBS.has(prefix[1])) shape = prefix[2];
