import type { ApiMapFile, EndpointNode, FieldNode } from '@junixlabs/apiflow-map';
import { fieldId } from '@junixlabs/apiflow-map';
...
export interface ProbeSample {
  ...
  // `{param}`. `url` is what was actually sent.
  // cm:why Without it a sample of `/api/v1/orders/{param}` cannot be reproduced: nobody can tell
  // which id answered, so a 200 is a claim rather than evidence.
  // cm:edge contract -> packages/cli/src/commands/probe.ts#runLive — set from the filled target,
  // never hand-written.
  url?: string;
}
