...
`diffMaps` has been in `packages/map` since before this, reachable from two places and neither of
them a file a user could point at: `check`, which re-scans the source first, and the workspace
server's history pane, which diffs two entries apiflow itself wrote. `check` also **refuses** a map
whose generator it does not recognise (`Cannot check a map generated by hand-written/1.`) — right for
`check`, and what put a hand-written map out of reach of every comparison the product owns.
...
`check` and `diff` print **one** panel. `renderMapDiff` is what `check` was already printing, lifted
out with the two sides passed in by the caller — "the map" and "the code" for `check`, the two files
...
that were swapped. The set difference now answers that case: `The same number of endpoints, but not
the same endpoints.` It is a **fallback**, reached only where the headline would otherwise have been
