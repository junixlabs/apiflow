...
### Added — `apiflow diff`, so a map can be a contract instead of a report

`apiflow diff <before.apimap> <after.apimap>` compares two map **files**. No source, no scanner, and
no rule about where either map came from. It exits **1** when the counted surface differs, **0** when
it does not, and **2** when it cannot answer at all — a missing file, an unparseable map, a missing
argument. Exit 2 is separate on purpose: a CI step that reads every non-zero as drift would report an
unreadable path as a broken contract. `--json` prints the same verdict with the whole `MapDiff` under
it.

`diffMaps` has been in `packages/map` since before this and had exactly one caller. `check` bound two
jobs together — re-scan the source, then diff and report — so the diff half could only ever be
reached by re-scanning, and `check` **refuses** a map whose generator it does not recognise
(`Cannot check a map generated by hand-written/1.`). That refusal is right for `check` and it is what
put a hand-written map out of reach of every comparison the product owns.

What that opens is the case the command was built for: write `be.apimap` and `fe.apimap` **before**
the source exists, use them to settle the API, then diff them against a real `scan-be` / `scan-fe`
once it is built. That is the difference between having drawn a plan and being able to show the build
matches it. `docs/guide/06-diff-two-maps.md` is the replayed transcript.

There is **one** renderer. `renderMapDiff` is the panel `check` was already printing, lifted out with
the two sides passed in by the caller — "the map" and "the code" for `check`, the two files for
`diff`. Two presentations of one `MapDiff` would be two answers to one question, and nothing would
tell a reader which was the real one.
...
something not in the contract", which is most of what a contract is bought to catch. Both are counted
now, both are part of divergence, and `check` prints them too. Divergence itself is one exported
predicate, `diverged()`, rather than an expression living inside `checkAgainst` where a second copy
would have grown next to it.

And `headlineFor` measured endpoint **count**, never endpoint identity. A build that dropped
`POST /users` and grew `DELETE /users/{param}` is three endpoints before and three after, every other
counter flat — so the headline read `No meaningful change.` directly above the list of the two routes
that were swapped. It now answers the set difference first: `The same number of endpoints, but not
the same endpoints.`
